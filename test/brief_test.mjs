...
// the same way every time. At RUNS=1 with 8 hostile probes + 3 sample posts, that's 11 live calls,
// each well under $0.001. parseBrief is called both with and without the post, so a run where the
...
// One call per post per run, capped at 50 so a stray RUNS value can't run away with the budget.
const CALL_BUDGET = Math.min(50, (hostile.length + clean.length) * RUNS);
